...
namespace PP.Bff.Identidades.Services
{
    ...
    {
        Task<IEnumerable<PermissaoDTO>> ObterPermissao(TipoUsuario tipoUsuario, string token);
    }
    ...
    public class PermissaoService : Service, IPermissaoService {
        ...
        public async Task<IEnumerable<PermissaoDTO>> ObterPermissao(TipoUsuario tipoUsuario, string token) {
            ...
            return await DeserializarObjetoResponse<IEnumerable<PermissaoDTO>>(identidade?.Content);
        }
    }
}